...
class XpCardControllerTest {
    ...
    void emit() throws Exception {
        when(service.emit(any(), anyInt())).thenReturn(new XpCard(UUID.randomUUID(), 500));
    ...
    void listCard() throws Exception {
        when(service.list()).thenReturn(List.of(new XpCard(UUID.randomUUID(), 500)));
    ...
    void getCard() throws Exception {
        ...
        when(service.get(any())).thenReturn(new XpCard(cardId, 500));
        when(service.getTransactions(any())).thenReturn(List.of(new XpCardTransaction(cardId, 500, "Gains 500 XP Points")));
